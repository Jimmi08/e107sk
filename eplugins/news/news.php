...
//	private $interval = 1;
	...
	function __construct()
	{
		...
		$this->ix = e107::getSingleton("news");
	...
	*/
	public function render($return = false)
	{
		...
		if($this->caption !== null) // always use tablerender with 2.3 theme spec
		{
	...
	private function setActions()
	{
		...
		$this->subSef = e107::getParser()->filter($sub_sef);
	...
	private function setRoute()
	{
		...
		if($this->subAction)
		{
			...
			switch ($this->action)
			{
				...
				case 'item':
					$this->newsUrlparms['news_id'] = $this->subAction;
					$this->newsUrlparms['news_sef'] = $this->subSef;
					$newsRoute = 'view/item';
				break;

				case 'tag':
					$this->newsUrlparms['tag'] = $this->subAction;
				 
					$newsRoute = 'list/tag';
					break;
 	
				default:
					$newsRoute = 'list/items';  //change to list/index
				break;
			}
		}
	...
	//	$i = $this->interval;
		 ...
		 $sc = e107::getScBatch('news_archive', 'news');
	...
	private function renderListTemplate()
	{
		...
		if ($this->action == 'all' OR $this->action == 'index' ) // show archive of all news items using list-style template.
		{
			$renTypeQry = '';
		...
		elseif ($this->action == 'category') // show archive of all news items in a particular category using list-style template.
		{
		...
		{
			$tagsearch =  $_GET['tag'] ;
			$tagsearch2 = str_replace('_', ' ',$tagsearch);
	...
	//	}
		...
		$text .= $tp->parseTemplate($template['end'], FALSE, $nsc);
	...
	//	}
			...
			*/
			...
			while(isset($newsAr[$i]) && $i <= $interval)
			{
				...
				{
					$socialArray = array('url'=>e107::url('news', 'item', $news, 'full=1'), 'title'=>$tp->toText($news['news_title']), 'tags'=>$news['news_meta_keywords']);
	...
	private function getPaginationParms($total, $amount)
	{
		...
		$opts = [
			...
			'amount'      => (int) $amount,
			'current'     => $this->from,
			'url'         => e107::url('news', $this->action, $this->newsUrlparms, array('query' => array('page' => '--FROM--'))),
		];
